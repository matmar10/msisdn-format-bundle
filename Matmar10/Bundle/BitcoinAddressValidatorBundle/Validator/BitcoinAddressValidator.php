...
namespace Matmar10\Bundle\BitcoinAddressValidatorBundle\Validator;
...
class BitcoinAddressValidator extends ConstraintValidator
{
    ...
    const ADDRESS_VERSION ='00';

    public function validate($value, Constraint $constraint)
    {

        try {
            $isValid = $this->checkAddress($value);
        } catch(\Exception $e) {
            $isValid = false;
        }
    ...
    public function hash160ToAddress($hash160, $addressversion = self::ADDRESS_VERSION)
    {
    ...
    public function checkAddress($addr, $addressversion = self::ADDRESS_VERSION)
    {
        $addr = $this->decodeBase58($addr);
        if (strlen($addr) != 50) {
            return false;
